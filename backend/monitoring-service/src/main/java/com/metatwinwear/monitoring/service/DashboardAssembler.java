...
public class DashboardAssembler {
    public DashboardSnapshot assemble(MonitoringRun run, ConfigurationRevision config, List<TelemetrySample> samples) {
        ...
        WearState wear = new WearState(latest.wearValue, SampleGenerator.WEAR_THRESHOLD, latest.wearRate,
                latest.remainingLife, latest.stage, latest.status);
        ...
        boolean danger = "danger".equals(wear.status());
        List<Recommendation> recommendations = List.of(
                new Recommendation("当前状态", wear.stage(), danger ? "danger" : "warning"),
                new Recommendation("风险等级", danger ? "高" : "较高", danger ? "danger" : "warning"),
                new Recommendation("剩余寿命", "≤ " + Math.max(5, (int) Math.ceil(wear.remainingLife())) + " min", "danger"),
                new Recommendation("建议", "完成当前加工后更换刀具", "info"),
                new Recommendation("建议剩余加工时间", "≤ 5 min", "warning"),
