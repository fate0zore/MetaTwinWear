...
const option = computed<EChartsOption>(() => {
  ...
  return {
    ...
    tooltip: { trigger: 'axis', appendTo: 'body', backgroundColor: '#071b2b', borderColor: '#12648b', textStyle: { color: '#bfefff' } },
    xAxis: { type: 'category', boundaryGap: false, data: channels[0].data.map((item) => item.time), axisLabel: { color: '#60859b', fontSize: 12 }, axisLine: { lineStyle: { color: '#174a67' } }, splitLine: { show: false } },
    yAxis: { type: 'value', scale: true, axisLabel: { color: '#60859b', fontSize: 12 }, axisLine: { show: false }, splitLine: { lineStyle: { color: 'rgba(43, 104, 137, .2)' } } },
